...
@inbounds function get_propagators(; Hamiltonian::Matrix{ComplexF64}, Jw::Vector{T}, β, dt, ntimes, rmax, build_propagator, cutoff=0.0, svec=[1.0 -1.0], verbose::Bool=false) where {T<:SpectralDensities.SpectralDensity}
...
@inbounds function get_propagators_approx(; Hamiltonian::Matrix{ComplexF64}, Jw::Vector{T}, β, dt, ntimes, rmax, build_propagator, cutoff=0.0, svec=[1.0 -1.0], verbose::Bool=false) where {T<:SpectralDensities.SpectralDensity}
...
@inbounds function propagate(; Hamiltonian::Matrix{ComplexF64}, Jw::Vector{T}, β::Real, ρ0::Matrix{ComplexF64}, dt::Real, ntimes::Int, rmax::Int, build_propagator, cutoff=0.0, svec=[1.0 -1.0], approx::Bool=false, verbose::Bool=false) where {T<:SpectralDensities.SpectralDensity}
