...
function build_augmented_propagator(; fbU::Array{ComplexF64, 3}, Jw::Vector{T}, β::Real, dt::Real, ntimes::Int, svec=[1.0 -1.0], reference_prop=false, extraargs::TNPIArgs, end_prop=false) where {T<:SpectralDensities.SpectralDensity}
...
function propagate(; fbU::Array{ComplexF64, 3}, Jw::Vector{T}, β::Real, ρ0::Matrix{ComplexF64}, dt::Real, ntimes::Int, kmax::Int, extraargs::TNPIArgs = TNPIArgs(), svec=[1.0 -1.0], reference_prop=false, verbose::Bool=false) where {T<:SpectralDensities.SpectralDensity}
    U0e = build_augmented_propagator(; fbU, Jw, β, dt, ntimes, extraargs, svec, reference_prop)
