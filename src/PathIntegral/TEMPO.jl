...
function build_augmented_propagator(; fbU::Array{<:Complex,3}, Jw::Vector{T}, β::Real, dt::Real, ntimes::Int, kmax::Union{Int,Nothing}=nothing, svec=[1.0 -1.0], reference_prop=false, extraargs::TEMPOArgs=TEMPOArgs(), verbose::Bool=false, output::Union{Nothing,HDF5.Group}=nothing, from_TTM::Bool=false, exec=ThreadedEx(), kwargs...) where {T<:SpectralDensities.SpectralDensity}
    ...
    if verbose
        @info "Step = 1; max bond dimension = $(maxldim); avg bond dimension = $(round(avgldim; digits=3)); norm(Tmat) = $(round(norm(T0e[1, :, :]); digits=3)); time = $(round(time_taken; digits=3)) sec; memory allocated = $(round(memory_allocated / 1024^3; digits=3)) GiB; gc time = $(round(gc_time; digits=3)) sec"
    end
    if !isnothing(output)
        if !from_TTM
    ...
    for j = 2:nmem
        ...
        avgldim = sum(ldims) / length(ldims)
        if verbose
            @info "Step = $(j); max bond dimension = $(maxldim); avg bond dimension = $(round(avgldim; digits=3)); norm(Tmat) = $(round(norm(T0e[j, :, :]); digits=3)); time = $(round(time_taken; digits=3)) sec; memory allocated = $(round(memory_allocated / 1024^3; digits=3)) GiB; gc time = $(round(gc_time; digits=3)) sec"
        end
        if !isnothing(output)
            output["U0e"][j, :, :] = U0e[j, :, :]
            output["T0e"][j, :, :] = T0e[j, :, :]
    ...
    if !isnothing(kmax) && ntimes > kmax
        ...
        avgldim = sum(ldims) / length(ldims)
        if verbose
            @info "Step = $(kmax+1); max bond dimension = $(maxldim); avg bond dimension = $(round(avgldim; digits=3)); norm(Tmat) = $(round(norm(T0e[kmax+1, :, :]); digits=3)); time = $(round(time_taken; digits=3)) sec; memory allocated = $(round(memory_allocated / 1024^3; digits=3)) GiB; gc time = $(round(gc_time; digits=3)) sec"
        end
        if !isnothing(output)
